...
<!--    <pre>{{ JSON.stringify(users, null, 2) }}</pre>-->
    <br/>
    <div style="display: flex">
      <n-button type="info" @click="forbidUsers" :disabled="checkedRowKeys.length===0">
        禁用
      </n-button>
      &nbsp;&nbsp;
      <n-button type="info" @click="unmakeUsers" :disabled="checkedRowKeys.length===0">
        放行
      </n-button>
      &nbsp;&nbsp;
      <n-pagination
...
export default defineComponent({
  ...
  setup() {
        ...
        render (user, index) {
          return h(MyButton, {
            ...
            onClick (data) {
              ...
              HTTP.PUT_AUTH("/admin/user/updateUserInfo",temUser).then(res=>{
                if(res.data.code === 0){
    ...
    function forbidUsers(){
      ...
      HTTP.PUT_AUTH("/admin/user/forbidUsers",checkedRowKeysRef.value).then(res=>{
        //设置选择的用户为空
    ...
    //解封用户
    function unmakeUsers() {
      console.log(checkedRowKeysRef.value)
      HTTP.PUT_AUTH("/admin/user/unmakeUsers",checkedRowKeysRef.value).then(res=>{
        //设置选择的用户为空
        checkedRowKeysRef.value = []
        if(res.data.code === 0){
          message.success(res.data.msg)
